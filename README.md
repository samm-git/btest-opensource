...
6. From time to time (~1 per second) server sends 12 bytes messages started with `07`, e.g. `07:00:00:00:01:00:00:00:36:6e:03:00`. It looks like to be server-level statistic, where:
  - stat[0] is 07 (command id)
  - stat[4] is sequence number
  - stat[8-11] number of bytes transferred (per sequence?) 
